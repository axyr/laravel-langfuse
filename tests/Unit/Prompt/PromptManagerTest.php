...
it('registers the resolved prompt for generation linking', function () {
    $api = Mockery::mock(PromptApiClientInterface::class);
    ...
    $registry = new \Axyr\Langfuse\Prompt\CurrentPromptRegistry();
    $manager = new PromptManager($api, new PromptCache(), 60, $registry);

    $manager->get('movie-critic');
...
it('does not register fallback prompts for generation linking', function () {
    ...
    $manager->get('movie-critic', fallback: 'fallback text');
